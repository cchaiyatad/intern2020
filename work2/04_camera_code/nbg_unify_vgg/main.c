...
{
    return vnn_PostProcessVgg( graph, priorPath,threshold );
}
    ...
    )
{
    ...
    int32_t i,loop;
    char *loop_s;
    // uint64_t tmsStart, tmsEnd, sigStart, sigEnd, msVal, usVal;
    ...
    )
{
     ...
     * argv0:   execute file
     * argv1:   data file
     * argv2:   priors.dat file
     * argv3:   threshold (0 - 1)
     * argv4~n: inputs n file
     */
    const char **inputs = (const char **)argv + 4;//2;
    uint32_t input_num = argc - 4;//2;

    if(vnn_UseImagePreprocessNode())
    {
        return vnn_PreProcessVgg_ImageProcess(graph, inputs, input_num);
    }
    return vnn_PreProcessVgg( graph, inputs, input_num );
}
    ...
    )
{
    ...
    // tmsStart = get_perf_count();
    graph = vnn_CreateVgg(data_file_name, NULL);
    ...
    )
{
    ...
    if(argc < 5)
    {
        printf("Usage: %s data_file.nb priors.dat inputs...\n", argv[0]);
        return -1;
    }
    ...
    // add priors.dat path argv[2]
    // add threshold in argv[3]
    status = vnn_PostProcessNeuralNetwork( graph, argv[2], argv[3] );
    TEST_CHECK_STATUS( status, final );
