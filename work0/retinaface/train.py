...
def train_net(args, ctx, pretrained, epoch, prefix, begin_epoch, end_epoch,
    ...
    # optimizer
    opt = optimizer.SGD(learning_rate=lr, momentum=0.9, wd=args.wd, rescale_grad=1.0/config.TRAIN.BATCH_IMAGES/len(ctx), clip_gradient=None)
    ...
    def _batch_callback(param):
      ...
      if mbatch==lr_steps[-1][0]:
        ...
        sys.exit(0)
    print(config.TRAIN.BATCH_IMAGES)
    print("\n\n","train batch size = ", config.TRAIN.BATCH_IMAGES* len(ctx),"\n\n")
    # train
